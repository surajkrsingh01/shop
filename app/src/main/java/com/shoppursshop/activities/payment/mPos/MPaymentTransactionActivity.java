...
public class MPaymentTransactionActivity extends BaseActivity implements PaymentTransactionConstants,
    ...
    private int deviceCommMode,devicename;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.i(TAG,"merchantRefNo "+merchantRefNo+" deviceCommMode "+deviceCommMode+" devicename "+devicename+
                " deviceMACAddress "+deviceMACAddress+" paymentType"+paymentType+" amount "+amount+" ");

        if (paymentType.equalsIgnoreCase(EMI)) {
            emivo = (AcquirerEmiDetailsVO) getIntent().getSerializableExtra("vo");
    ...
    public void deviceConnection(String address) {
        ...
        Log.i(TAG,"device connecting... "+address);
        ...
        if (paymentType.equalsIgnoreCase(EMI)) {
            if (paymentoptioncode.equalsIgnoreCase("emilist")) {
                ...
                initialization.initiateTransaction(handler, devicename, deviceMACAddress, amount, paymentType,
                        PaymentTransactionConstants.CREDIT, "9000000000", 71.000001, 17.000001, merchantRefNo, null,
                        emivo, deviceCommMode);
            } else {
                initialization = new PaymentInitialization(MPaymentTransactionActivity.this);
                initialization.initiateEMITransactionWithPaymentOptionCode(handler, devicename, deviceMACAddress,
                        amount, paymentType, PaymentTransactionConstants.CREDIT, "9000000000", 71.000001, 17.000001,
                        merchantRefNo, null, paymentoptioncode, deviceCommMode);
            }

        }
        else if(paymentType.equalsIgnoreCase("serialNumber"))
        {
            initialization = new PaymentInitialization(MPaymentTransactionActivity.this);
            ...
            Log.i(TAG,"initiating payment... "+address);
            try {
                initialization = new PaymentInitialization(MPaymentTransactionActivity.this);
                initialization.initiateTransaction(handler, devicename, address, amount, paymentType, null,
                        "9718181697", 71.000001, 17.000001, merchantRefNo, null, deviceCommMode);
    ...
    private void voidTransaction(String rrn) {
        try {
            ...
            initialization.initiateVoidTransaction(handler, rrn);
        } catch (RuntimeException e) {
            e.printStackTrace();
        }
    }

    private void getTransDetails(TransactionVO vo){
        try {
    ...
    private final Handler handler = new Handler() {
        ...
        public void handleMessage(android.os.Message msg) {
            ...
            if (msg.what == QPOS_ID) {
                Toast.makeText(MPaymentTransactionActivity.this, (String) msg.obj, Toast.LENGTH_LONG).show();
                ...
                try {
                    ...
                    data = new ObjectMapper().writerWithDefaultPrettyPrinter().writeValueAsString(vo);
                } catch (JsonGenerationException e1) {
                    e1.printStackTrace();
                } catch (JsonMappingException e1) {
                    e1.printStackTrace();
            ...
            else if (msg.what == TRANSACTION_FAILED || msg.what == CHIP_TRANSACTION_DECLINED
                    || msg.what == SWIP_TRANSACTION_DECLINED) {
                TransactionVO vo = (TransactionVO) msg.obj;
                /*
                 * Toast.makeText(PaymentTransactionActivity.this,
                 * "Transaction Status : " + vo.getStatus(),
                ...
                /*
                 * Toast.makeText(PaymentTransactionActivity.
                 * this,"Error message"+ (String) msg.obj, 1000).show();
                 * finish();
                 */
                alertMessage((String) msg.obj);
            } else if (msg.what == TRANSACTION_PENDING) {
                Toast.makeText(MPaymentTransactionActivity.this, (String) msg.obj + "Pending status", Toast.LENGTH_LONG).show();
                finish();
               ...
               /* ArrayList<String> applicationList = (ArrayList<String>) msg.obj;
                ...
                emvList.setOnItemClickListener(new AdapterView.OnItemClickListener() {
